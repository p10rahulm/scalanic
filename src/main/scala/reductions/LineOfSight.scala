...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var max_so_far:Float = 0
    var index: Int = 0
    while(index < input.length) {
      if(input(index)/index > max_so_far) {
        max_so_far = input(index)/index
      }
      output(index) = max_so_far
      index = index +1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var index:Int  = from
    while(index<until){
      max_so_far = Math.max(input(index)/index, max_so_far)
      index = index + 1
    }
    max_so_far
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var max_so_far:Float = startingAngle
    var index:Int = from
    while(index<until){
      if(max_so_far<input(index)/index) max_so_far = input(index)/index
      output(index) = max_so_far
      index = index+1
    }
  }
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = {
    val upsweep_tree = upsweep(input,1,input.length,threshold)
    downsweep(input,output,1,upsweep_tree)
  }
}
